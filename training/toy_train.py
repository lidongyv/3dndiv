...
for epoch in range(opt.nepoch):
	...
	for i, data in enumerate(dataloader, 0):
		...
		train_data.append([points.transpose(2,1).contiguous()[0].data.cpu().numpy(), \
		pointsReconstructed1[0].data.cpu().numpy(), \
		pointsReconstructed2[0].data.cpu().numpy(), \
		...
		if i%1 <= 0:
			...
			# print(train_data[-1][1]==pointsReconstructed1[0].data.cpu())
		...
		for eval_data in range(2):
			...
			with torch.no_grad():
				...
				test_data.append([points.transpose(2,1).contiguous()[0].data.cpu().numpy(), \
				pointsReconstructed1[0].data.cpu().numpy(), \
				pointsReconstructed2[0].data.cpu().numpy(), \
				pointsReconstructed3[0].data.cpu().numpy()])
				# VIZUALIZE
