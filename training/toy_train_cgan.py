...
parser.add_argument('--env', type=str, default ="toyatlascgan"   ,  help='visdom environment')
...
test_data=[]
BCE = nn.BCELoss()
real_label=torch.ones(opt.batchSize).cuda(1)
fake_label=torch.zeros(opt.batchSize).cuda(1)
weight=30
...
for epoch in range(opt.nepoch):
	...
	if epoch==100:
		...
		optimizerd = optim.Adam(discriminator.parameters(), lr = lrate/10.0)
	if epoch%30==0:
		weight/=2
	for i, data in enumerate(dataloader, 0):
		...
		pointsReconstructed1  = network1.forward_inference_cgan(points) #forward pass
		pointsReconstructed2  = network2.forward_inference_cgan(points) #forward pass
		pointsReconstructed3  = network3.forward_inference_cgan(points) #forward pass
		...
		d_fake2=discriminator(pointsReconstructed2.transpose(2,1).contiguous())
		d_fake3=discriminator(pointsReconstructed3.transpose(2,1).contiguous())
		loss_net=(BCE(d_fake1,real_label)+BCE(d_fake2,real_label)+BCE(d_fake3,real_label))/weight
		...
		print('[%d: %d/%d] G loss: %f D loss %f chamfer %f' %(epoch, i, len_dataset/32, loss_net.item(),err_D.item(),loss_net.item()))
	...
	torch.save(discriminator.state_dict(), '%s/discriminator.pth' % (dir_name))
